...
def build_lock(python: str) -> str:
    ...
    # ВНИМАНИЕ: шапка замка — ТОЛЬКО ASCII, хотя весь остальной проект пишет
    # по-русски. Замок читают чужие средства (pip-audit, сканеры состава), а
    # разбор requirements-файлов в pip_requirements_parser декодирует их по
    # ЛОКАЛИ: под русской Windows это cp1251, и кириллица в комментарии роняет
    # анализатор UnicodeDecodeError. Мы обещали службе ИБ ПРОВЕРЯЕМЫЙ состав,
    # значит файл обязан читаться без переменных окружения. Пояснения по-русски
    # живут в docs/enterprise-deploy.md, раздел 9.
    lines = [
        "# Pinned dependency set for Voice Input Local (US-089).",
        "#",
        "# DO NOT EDIT BY HAND. Generated by:",
        "#     .venv\\Scripts\\python.exe scripts\\make_lockfile.py",
        "#",
        "# Exact versions + hashes: building the same tag on different days yields",
        "# the same set, and a file swapped on a mirror will not install.",
        "# Version ranges stay in requirements.txt (intent); this file is the fact.",
        "# Update procedure: docs/enterprise-deploy.md, section 9.",
        "#",
        "# ASCII only on purpose: audit tools decode requirements files using the",
        "# system locale, so non-ASCII comments break them on non-UTF-8 machines.",
        "#",
        "# Generated: %s, packages: %d" % (date.today().isoformat(), len(rows)),
        "",
    ]
