...
### Централизованное обновление больше не срабатывает как антивирусная угроза (US-061)

- Убран механизм авто-перезапуска приложения после обновления: он запускал фоновый PowerShell, который поведенческий антивирус (Kaspersky) расценивал как троянское поведение и блокировал. Теперь при обновлении приложение **не запускает никаких процессов**.
- После тихого обновления приложение закрывается и **возвращается штатным автозапуском Windows** при следующем входе/перезагрузке (если включён «Запуск вместе с Windows»). Для немедленного открытия администратор может настроить в системе развёртывания отдельную задачу запуска приложения.
- Рабочие части сохранены: корректное определение «программа запущена» (US-058) и интерактивный выбор **«Закрыть и обновить / Отклонить»** при занятости (US-059).

> Убирает ложное срабатывание антивируса на промежуточных сборках 4.17.4–4.17.6 (как релиз они не выпускались).


## v4.17.6

### Централизованное обновление: рабочий детект занятости и «Закрыть и обновить» (US-058/US-059/US-060)

- **Исправлен детект «программа запущена» (US-058).** Установщик определял занятость `.exe` открытием на чтение — а у запущенной программы это проходит, поэтому при открытом простаивающем приложении обновление «молча ничего не делало». Теперь проверка — через попытку открыть на запись (у работающего процесса это даёт «файл занят»). Тихое обновление при открытой простаивающей программе (закрытие → установка → авто-перезапуск) наконец действительно работает.
- **«Закрыть и обновить» действительно ставит и перезапускает (US-059).** Раньше при обновлении во время активной работы кнопка «Закрыть и обновить» закрывала программу, но установка откладывалась до следующей попытки. Теперь установщик ждёт решения в том же прогоне: программа показывает «Обновление устанавливается…», закрывается, обновление ставится сразу, программа снова открывается. **Для ИТ:** таймаут задачи развёртывания в KSC должен быть больше времени ожидания решения (по умолчанию 3 минуты).
- **Надёжный авто-перезапуск (US-060).** Момент завершения установки определяется по явному сигналу, а не по времени модификации файла — перезапуск срабатывает сразу, в том числе при переустановке той же версии.

> Заменяет неработавшую 4.17.4 (установщик падал с ошибкой «app constant before it was initialized») и промежуточную 4.17.5.


## v4.17.5

### Исправление: установщик 4.17.4 не запускался (US-058, hotfix)

- Устранён сбой установщика **«Runtime error … An attempt was made to expand the "app" constant before it was initialized»**, из-за которого установка/обновление версии 4.17.4 прерывалась на большинстве компьютеров (версию 4.17.4 фактически нельзя было установить).
- Причина: в US-058 проверка занятости `.exe` в `InitializeSetup` обращалась к пути установки через константу `{app}`, которая на этом раннем этапе установки ещё не инициализирована. Теперь путь к установленному `.exe` берётся из реестра (запись Inno об установке, `Inno Setup: App Path`), без `{app}`.
- Функциональность US-058 (тихое централизованное обновление при открытом простаивающем приложении с показом процесса и авто-перезапуском) не изменилась.
