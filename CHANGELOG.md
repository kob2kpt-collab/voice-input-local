...
### Системные горячие клавиши без глобального перехвата клавиатуры

- Библиотека `keyboard` и её низкоуровневый клавиатурный хук полностью удалены из приложения, зависимостей и EXE-сборки. Вместо потока всех нажатий Windows сообщает приложению только о срабатывании явно зарегистрированной комбинации через `RegisterHotKey` / `WM_HOTKEY`.
- Устранён класс ошибок с «залипшим» внутренним состоянием клавиш после блокировки и разблокировки Windows: приложение больше не сопоставляет события нажатия и отпускания и не хранит карту нажатых клавиш.
- Режим Push-to-Talk удалён. Горячая клавиша работает только как переключатель: первое нажатие запускает запись, второе останавливает её. Старое поле `hotkey_mode` в `config.json` безопасно игнорируется.
- Поддерживаются комбинации из одной основной клавиши (буква A–Z, цифра 0–9, `Space` или `F1`–`F24`) и хотя бы одного модификатора `Ctrl`, `Alt`, `Shift` или `Win`. Новая комбинация регистрируется до снятия предыдущей, поэтому ошибка или конфликт не лишают пользователя рабочей горячей клавиши.
- Временная глобальная обработка `Esc` для отмены записи/распознавания также переведена на `RegisterHotKey` и снимается сразу после завершения операции.
